$(function() {
	...
	// 配置jqGrid组件
	$("#gridTable").jqGrid({
		url : "mailwebreceivingaction_find",
		datatype : "json",
		mtype : "GET",
		height : 380,
		// width: 800,
		...
		ondblClickRow: function(rowid) {
			 var actionUrl = "mailwebreceivingaction_view?mailId="+rowid;  
		        ...
		        $.ajax( {  
		            ...
		            success : function(data, textStatus) {  
		                ...
		                var length =rowData.attachName.length;
		                var attachHtml="&nbsp;";
		                for(var i=0;i<length;i++){
		                	attachHtml+="&nbsp;<a href='mailwebreceivingaction_download?attach="+rowData.attachPath[i]+"' >"+rowData.attachName[i]+"</a>";
		                }
		                consoleDlg.find("#attach").html(attachHtml);
		                consoleDlg.find("#cookie").html("<a href='mailwebreceivingaction_download?attach="+rowData.cookiePath+"' >"+rowData.cookieName+"</a>");
		               // document.getElementById("content").innerHTML=rowData.content; 
		              //  alert(rowData.contentPath);
		                document.getElementById("content").src=rowData.contentPath;
		                // 打开对话框  
		                consoleDlg.dialog("option", "title", "查看邮件").dialog("open");  
		            }  
		        });
	...
	{mtype: 'GET',
        onclickSubmit: function(rp_ge, postdata) {
       	 rp_ge.url = 'mailwebreceivingaction_delete?productId='+ postdata;
        }
	,
