$(function() {
	...
	$("#gridTable").jqGrid({
		...
		// ,
		// {name:"createdTime",index:"createdTime",label:"创建时间",formatter:'date',searchtype:"datetime",
		// formatoptions: {srcformat:'Y-m-d H:i:s',newformat:'Y-m-d
		// H:i:s'},sortable:false,stype:'text',
		// searchoptions:{dataInit:dateTimePick, attr:{title:'Select
		// Date'}},searchrules:{required:true} }
		],
		viewrecords : true,
		rowNum : 50,
		rowList : [ 50,100,200 ],
	...
	$("#gridTable").jqGrid('navGrid',"#gridPager").jqGrid('navButtonAdd',"#gridPager",{
		caption:"分配用户",
		onClickButton:loadUser
	});
...
var addrole = function() {
	...
	var params = {
	/*
	 * "role.code" : pcode, "role.name" : pname, "role.email" : pEmail,
	 * "role.phone" : pPhone
	 */
	};
			...
			.ajax({
				...
				success : function(data, textStatus) {
					if (data.ajaxResult == "success") {
						...
						if (srcrowid) {
							$("#gridTable").jqGrid("addRowData", data.role.id,
...
var deleteRole = function() {
	...
	var actionUrl = "roleaction_delete?ids=" + selectedRowId;
	$.ajax({
		type : "post",
		url : actionUrl,
		// data : params,
...
var loadUser = function() {
	...
	//alert(selectedRowId);
	window.open("roleuser.html?roleId="+selectedRowId,'newwindow', 'height=600, width=800, top=0, left=0, toolbar=no, menubar=no, scrollbars=no, resizable=no,location=n o, status=no');
