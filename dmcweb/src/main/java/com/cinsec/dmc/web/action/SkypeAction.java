...
@Controller
@Scope("prototype")
public class SkypeAction extends BaseAction<ImRecord> {
	...
	public String find() {
		baseService.writeOperLog("查看Skype记录信息");
	...
	public long getResultSize() {
		return baseService.getResultCount(ImRecord.class, "imClassify", 2);
	}

	@Override
	public List<ImRecord> listResults(int from, int length) {
		resultList = baseService.getResultList(ImRecord.class, "imClassify", 2,
				from, length);
		return resultList;
	}
	...
	public List<ImRecord> listResults(String groupOp, List<Criterion> criteria,
			int from, int length) {
		return baseService.getResultList(ImRecord.class, "imClassify", 2,
