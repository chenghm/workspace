...
@Controller
@Scope("prototype")
public class MsnAction extends BaseAction<ImRecord> {
	...
	public String find() {
		baseService.writeOperLog("查看MSN记录信息");
	...
	public long getResultSize() {
		return baseService.getResultCount(ImRecord.class, "imClassify", 1);
	}

	@Override
	public List<ImRecord> listResults(int from, int length) {
		resultList = baseService.getResultList(ImRecord.class, "imClassify", 1,
				from, length);
		return resultList;
	}
	...
	public List<ImRecord> listResults(String groupOp, List<Criterion> criteria,
			int from, int length) {
		return baseService.getResultList(ImRecord.class, "imClassify", 1,
